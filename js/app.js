const STORAGE_KEY = '***';
        ...
        // 2. Molekül Verisi Çek ve Yükle
        async function loadMolecule(name, trName = '') {
          showLoading(true, `"${name}" 3D koordinatları PubChem'den çekiliyor...`);

          try {
            const res = await fetch(`/api/molekul/data?name=${encodeURIComponent(name)}`);
            const data = await res.json();

            if (!data.success || !data.sdf) {
              throw new Error(data.error || 'Molekül bulunamadı');
            }

            currentSdfData = data.sdf;
            currentMolInfo = {
              name: data.name,
              trName: trName || data.name,
              formula: data.formula || 'Bileşik',
              weight: data.weight ? data.weight + ' g/mol' : '-',
              iupac: data.iupac || data.name,
              cid: data.cid || '-'
            };

            updateInfoCard();
            renderModelInViewer(data.sdf);
        ...
        // 4. Kişisel Laboratuvarım (LocalStorage & SSO)
